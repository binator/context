...
impl<Context, Atom: Into<Context>> Add<Atom> for Tree<Context> {
  ...
  fn add(self, context: Atom) -> Self {
    ...
    match self {
      one @ Tree::Element(..) => Self::And(vec![one, Self::Element(rhs)]),
      Tree::And(stack) => Self::And(stack.acc(Self::Element(rhs))),
      tree @ Tree::Or(_) => Self::And(vec![tree, Self::Element(rhs)]),
    }
  }
}
...
impl<Context> BitOr for Tree<Context> {
  ...
  fn bitor(self, other: Self) -> Self {
    match self {
      one @ Tree::Element(..) => Self::Or(vec![one, other]),
      stack @ Tree::And(..) => Self::Or(vec![stack, other]),
